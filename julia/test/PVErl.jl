...
fault3 = NodeParameterChange(node = "bus1", value = 0.95, tspan_fault = (10.,20.),var=:V)
fault4 = NodeParameterChange(node = "bus2", value = 0.9, tspan_fault = (10.,20.),var=:E_f)
#sol= simulate(fault2, pg2, insert!(operationpoint[:],6,0.0), timespan)
sol1= simulate(fault3, pg1, insert!(operationpoint[:],size(operationpoint[:])[1],-0.1), timespan)
...
plot_v1=plot(sol1.dqsol.t,v_sim1',label=["PV" "Fourth"],title="V")
#plot_p = plot(sol.dqsol.t,p_sim',label=["Slack" "Fourth"])
plot_p1=plot(sol1.dqsol.t,p_sim1',label=["PV" "Fourth"],title="P")
