...
pq_list = [ODEVertex(; f=PQVertex(1 + 0 * im),
                     dim=2,
                     mass_matrix=0.0,
                     sym=[:v_r, :v_i])
           for i in 1:5]

swing_list = [ODEVertex(; f   = SwingVertex(-1,0.01),
                        dim = 3,
                        sym = [:v_r, :v_i, :ω])
              for i in 1:5]
...
power_network_rhs = network_dynamics(vertex_list, edge_list, graph)
ic = find_valid_ic(power_network_rhs, rand(25))
test_prob = ODEProblem(power_network_rhs, ic, (0.0, 20.0))
