...
buses = OrderedDict(
      ...
      "bus2" => SwingEqLVS(H=5., P=0.1, D=1., Ω=50., Γ=20., V=1.0), 
      );  

buses1 = OrderedDict(
      "bus1" => PVAlgebraic(P=-0.1,V=1.0),
      "bus2" => SwingEqLVS(H=5., P=0.1, D=1., Ω=50., Γ=20., V=1.0), 
      );  
      
buses2 = OrderedDict(
        "bus1" => PVAlgebraic(P=-0.1,V=1.0),
        "bus2" => FourthOrderEq(T_d_dash=2.4, D=1., X_d=0.8979, X_q=0.646, Ω=50, X_d_dash=0.2995, T_q_dash=0.1, X_q_dash=0.646, P=1., H=2.148, E_f=1)
        );  
...
timespan= (0.0,10.)
fault = PowerPerturbation(node= "bus1",fault_power=-0.2,tspan_fault = (2.,5.),var=:P)
...
#=
plot_v= plot(sol.dqsol.t,v_sim',label=["Slack" "Swing"])
plot_p = plot(sol.dqsol.t,p_sim',label=["Slack" "Swing"])
plot_ω=plot(sol.dqsol.t,(ω_sim')/(2*pi),label=[ "Swing"],title="f")
